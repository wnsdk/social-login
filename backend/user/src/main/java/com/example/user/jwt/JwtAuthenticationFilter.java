        ...
        try {
            ...
            // AccessToken 유효하지 않다면 (RefreshToken 확인)
                ...
                Cookie cookie = CookieUtil.getCookie(request, REFRESH_TOKEN_COOKIE_NAME.getValue()).orElseThrow(() ->
                        new BaseException(ErrorMessage.INVALID_REFRESH_TOKEN));
                String refreshTokenFromClient = cookie.getValue();

                // Redis 에서 RefreshToken 추출
                String id = jwtProvider.getSub(refreshTokenFromClient);
                String refreshTokenFromServer = redisTemplate.opsForValue().get(REFRESH_TOKEN_REDIS_NAME + id);
...
//                    context.setAuthentication(null);    // TODO : 아니 context 왜 처음부터 authentication 값 제대로 들어가있냐? 언제 들어감?? 원래 이 코드 없어도 됐는데..
                    throw new BaseException(ErrorMessage.INVALID_REFRESH_TOKEN);
                }
            }
        } catch (Exception e) {
